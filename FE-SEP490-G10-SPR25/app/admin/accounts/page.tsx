...
import { Table, Button, Modal, Form, Input, Select, Switch, Space, message, Popconfirm, DatePicker, Tabs } from 'antd';
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      if (editingUserId) {
        ...
        const response = await adminService.updateAccount(editingUserId, accountData);
        console.log('API update response:', response);
        
        if (hasNewPassword) {
          message.success('Tài khoản đã được cập nhật thành công cùng với mật khẩu mới');
        } else {
        ...
        const result = await adminService.createReceptionistAccount(accountData);
        console.log('API create receptionist response:', result);
        if (result) message.success('Receptionist account created successfully');
      }
    ...
    <div className="p-6">
      ...
      >
        ...
        >
          ...
          >
            <Input.Password placeholder={editingUserId ? "Để trống để giữ mật khẩu hiện tại" : "Nhập mật khẩu (ít nhất 6 ký tự)"} />
          </Form.Item>
              ...
              { required: true, message: 'Please input citizen ID!' },
              {
                validator: (_, value) => {
                  ...
                  if (!Number.isInteger(num) || num <= 0) {
                    return Promise.reject('Citizen ID must be a positive integer');
                  }
                  return Promise.resolve();
                }
              }
