...
export const ChatBot = () => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let errorText =
        "Xin lỗi, tôi không thể xử lý yêu cầu của bạn vào lúc này. Vui lòng thử lại sau.";

      if (errorCount >= 2) {
        errorText =
          "Hệ thống đang gặp sự cố. Vui lòng thử một trong các cách sau:\n\n" +
          ...
          <div className="flex-1 p-5 overflow-y-auto">
            {messages.map((msg, index) => (
              <div
                ...
                className={`mb-4 ${
                  msg.isUser
                    ? "ml-auto mr-0 bg-cyan-600 text-white"
                    : "ml-0 mr-auto bg-gray-200 text-gray-800"
                } rounded-lg p-3 max-w-[85%] break-words whitespace-pre-line text-base ${
                  msg.source === "fallback"
                    ? "border-l-4 border-yellow-500"
                    : ""
                } ${msg.source === "error" ? "border-l-4 border-red-500" : ""}`}
              >
                <p>{msg.text}</p>
                <span className="text-xs opacity-70 block mt-1 flex items-center">
                  ...
                  {msg.source === "fallback" && (
                    <span className="ml-2 bg-yellow-200 text-yellow-800 text-xs px-1 rounded">
                      Fallback
                    </span>
                  )}
                </span>
            ...
            {isLoading && (
              <div className="flex items-center space-x-2 ml-0 mr-auto bg-gray-200 text-gray-800 rounded-lg p-3">
                <div
                  className="w-3 h-3 rounded-full bg-gray-500 animate-bounce"
