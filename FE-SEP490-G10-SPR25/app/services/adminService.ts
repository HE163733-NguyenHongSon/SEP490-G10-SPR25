...
export const adminService = {
  ...
  async createDoctorAccount(accountData: AdminDTO): Promise<IUser | null> {
    try {
      const formattedData = {
        ...accountData,
        citizenId: typeof accountData.citizenId === 'string' ? parseInt(accountData.citizenId) : accountData.citizenId
      };
      
      console.log('Creating doctor account with data:', formattedData);
      const response = await axios.post(`${API_URL}/api/Admin/accounts/doctor`, formattedData);
      return response.data;
    } catch (error) {
  ...
  async createReceptionistAccount(accountData: AdminDTO): Promise<IUser | null> {
    try {
      const formattedData = {
        ...accountData,
        citizenId: typeof accountData.citizenId === 'string' ? parseInt(accountData.citizenId) : accountData.citizenId
      ...
      const response = await axios.post(`${API_URL}/api/Admin/accounts/receptionist`, formattedData);
      return response.data;
    } catch (error: any) {
  ...
  async updateAccount(userId: number, accountData: AdminDTO): Promise<IUser> {
    try {
      ...
      const formattedData: any = {
        ...
        citizenId: typeof accountData.citizenId === 'string' ? parseInt(accountData.citizenId as string) : accountData.citizenId,
      ...
      const requiredFields = ['userName', 'email', 'phone', 'gender', 'dob', 'citizenId'];
      for (const field of requiredFields) {
        ...
        if (formattedData[key] === undefined || formattedData[key] === null || formattedData[key] === '') {
          console.warn(`Warning: Required field '${field}' may be missing or empty in request`);
        }
      }
      ...
      const response = await axios.put(`${API_URL}/api/Admin/accounts/${userId}`, formattedData, {
        headers: {
      ...
      if (error.response) {
        ...
        if (typeof error.response.data === 'string') {
          ...
          console.error('Error response data (object):', JSON.stringify(error.response.data, null, 2));
  ...
  async toggleAccountStatus(userId: number, isActive: boolean): Promise<boolean> {
    try {
      const response = await axios.patch(`${API_URL}/api/Admin/accounts/${userId}/status?isActive=${isActive}`);
      return response.data;
    } catch (error) {
  ...
  async assignRole(userId: number, roleId: number): Promise<boolean> {
    try {
      const response = await axios.post(`${API_URL}/api/Admin/accounts/${userId}/roles/${roleId}`);
      return response.data;
    } catch (error) {
  ...
  async removeRole(userId: number, roleId: number): Promise<boolean> {
    try {
      const response = await axios.delete(`${API_URL}/api/Admin/accounts/${userId}/roles/${roleId}`);
      return response.data;
    } catch (error) {
